...
class Trainer:
    def __init__(self, height=64, width=64, epochs=50000, batch=32, checkpoint=50, train_data_path_A='',
                 train_data_path_B='', test_data_path_A='', test_data_path_B='', lambda_cycle=10.0, lambda_id=1.0):
        self.EPOCHS = epochs
        self.BATCH = batch
        ...
        # 几个核心思想
        # 1.需要确保原始的A图片和B图片以Keras的形式存储在Input类中，变量orig_A和orig_B将会作为输入值，在之后的三个组件中共享
        # 2.fake_A和fake_B是生成器从一个风格到另一个风格转换生成的图片，因此称其为fake
        # 3.reconstructed_A和reconstructed_B以fake_A和fake_B作为输入并将其重新转换为原始图片的风格
        # id_A和id_B完成相同功能，它们以原始图片作为输入并转换为相同的风格。在理想情况下，这两个函数不因该对图片进行任何修改
        ...
        self.gan = GAN(model_inputs=model_inputs, model_outputs=model_outputs, lambda_cycle=lambda_cycle,
                       lambda_id=lambda_id)
    ...
    def train(self):
        for e in range(self.EPOCHS):
            ...
            while min(len(X_train_A_temp), len(X_train_B_temp)) > self.BATCH:
                ...
                X_train_A_temp = np.delete(X_train_A_temp,
                                           range(starting_indexs, (starting_indexs + count_real_images)), 0)
                X_train_B_temp = np.delete(X_train_B_temp,
                                           range(starting_indexs, (starting_indexs + count_real_images)), 0)
                ...
                print('Batch: ' + str(int(b)) + ', [Discriminator_A :: Loss: ' + str(
                    discriminator_loss_A) + '], [ Generator :: Loss: ' + str(generator_loss) + ']')
                if b % self.CHECKPOINT == 0:
                    label = str(e) + '_' + str(b)
                    self.plot_checkpoint(label)

            print('Epoch: ' + str(int(e)) + ', [Discriminator_A :: Loss: ' + str(
                discriminator_loss_A) + '], [ Generator :: Loss: ' + str(generator_loss) + ']')
